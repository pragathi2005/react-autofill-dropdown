    ...
    <div style={{ padding: '20px', maxWidth: 400 }}>
      ...
      <input
        ...
        onChange={e => {
          ...
          setSelectedTopic(null);  // reset selected on new search
        }}
        style={{ width: '100%', padding: '8px', boxSizing: 'border-box' }}
      />

      {query && (
        <ul style={{ listStyleType: 'none', padding: 0, marginTop: 5, maxHeight: 150, overflowY: 'auto', border: '1px solid #ccc' }}>
          {filteredTopics.length > 0 ? (
            filteredTopics.map(topic => (
              ...
              >
                {highlightMatch(topic.name, query)}
              </li>
            ))
          ) : (
            <li style={{ padding: '8px', color: '#888' }}>No topics found</li>
          )}
        </ul>
      )}
      ...
      {selectedTopic && (
        <p style={{ marginTop: '20px' }}>
          You selected: <b>{selectedTopic}</b>
        </p>
      )}
    </div>
  );
